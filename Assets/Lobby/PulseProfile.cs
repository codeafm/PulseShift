...
[Serializable] public class PulseProfile {
 ...
 public int crystals,resonance,wins,bestEndless,selectedSkin,selectedPortal,coins,giftStreak,crystalRewardsToday,resonanceRewardsToday;
 ...
 public int quality=2,profileVersion=2;
 public int[] firstWins=new int[0];
 public static PulseProfile Load(bool testing){
  if(testing)return new PulseProfile();
  try{var p=JsonUtility.FromJson<PulseProfile>(PlayerPrefs.GetString("PulseLobby.Profile.v1",""));if(p!=null){p.firstWins=p.firstWins??new int[0];p.playerName=string.IsNullOrWhiteSpace(p.playerName)?"Искра":p.playerName;p.crystals=Mathf.Max(0,p.crystals);p.resonance=Mathf.Max(0,p.resonance);p.coins=Mathf.Max(0,p.coins);p.ownedSkins|=1;p.ownedPortals|=1;if(p.selectedSkin<0||p.selectedSkin>4||(p.ownedSkins&(1<<p.selectedSkin))==0)p.selectedSkin=0;if(p.selectedPortal<0||p.selectedPortal>4||(p.ownedPortals&(1<<p.selectedPortal))==0)p.selectedPortal=0;p.sound=Mathf.Clamp01(p.sound);p.music=Mathf.Clamp01(p.music);if(p.profileVersion<3){p.vibration=true;p.fps60=true;p.quality=2;p.ownedPortals|=1;p.profileVersion=3;}p.quality=Mathf.Clamp(p.quality,0,2);return p;}}catch(Exception e){Debug.LogWarning("Profile could not be read; existing stored data retained: "+e.Message);}
 ...
 public bool ClaimGift(string day){if(!GiftReady(day))return false;DateTime today;if(!DateTime.TryParse(day,out today))today=DateTime.UtcNow.Date;DateTime prior;giftStreak=DateTime.TryParse(giftDay,out prior)&&prior.Date==today.Date.AddDays(-1)?Mathf.Min(7,giftStreak+1):1;giftDay=day;crystals+=100;resonance+=1;coins+=500;return true;}
